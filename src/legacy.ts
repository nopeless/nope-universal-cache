...
const STARTED_TIMESTAMP = Date.now();
...
class MemoryCache<T> extends BaseCache implements CacheInterface<T> {
  ...
  async set(
    ...
    this.mapping.set(key, data(value, timeout, ttl));
    return value;
  }
  ...
  async expiresAt(key?: string): Promise<number> {
    if (key) {
      const data = this.mapping.get(key);
      if (data) {
        return data.createdAt + (data.ttl ?? this.ttl);
      }
    }
    return 0;
  }
...
class FileSystemCache<T> extends BaseCache implements CacheInterface<T> {
  ...
  async unlink(hash: string): ReturnType<typeof fsp[`unlink`]> {
    console.debug(`deleing cache`, hash);
    return fsp
  ...
  async read(
    ...
    console.debug(`reading from cache`, hash);
  ...
  async write(
    ...
    console.debug(`writing to cache`, hash);
    return fsp
  ...
  async get(key: string): Promise<T | undefined> {
    ...
    // This sometimes happens when some other cache is writing
  ...
  async expiresAt(key?: string): Promise<number> {
    if (key) {
      const hash = this.hashFunction(key);
      const timeout = this.mapping.get(hash);
      if (timeout) return (timeout as Timeout)._idleTimeout + STARTED_TIMESTAMP;
    }
    return 0;
  }
...
class CacheManager<T> implements CacheInterface<T | Promise<T>> {
  ...
  async get(key: string): Promise<T | undefined> {
    ...
    const expirations = this.caches.map((c) => c.expiresAt(key));
    let i = 0;
    let v;
    ...
    promises.push(
      Promise.all(expirations.slice(hitIndex)).then((values) => {
        ...
        const maxExpiration = values[maxIndex];
        ...
        for (; hi < caches.length; hi++) {
          ...
          const c = caches[hi];
          promises.push(
            c.updateTtl(key, Math.min(maxExpiration, values[hi]) - Date.now())
          );
        }
        hi++;
  ...
  async expiresAt(key?: string): Promise<number> {
    return Promise.all(this.caches.map((c) => c.expiresAt(key))).then(
  ...
  async updateTtl(key: string, ttl?: TtlOption): Promise<void> {
    ttl = msForTtl(ttl);
    const finalTtl = ttl ?? (await this.expiresAt()) - Date.now();
